...
def optimize_model():
    global last_sync
    if len(memory) < BATCH_SIZE:   #Start the optimisation only when we full the batch
        return
    transitions = memory.sample(BATCH_SIZE)  #Sample of memory : Size = batch
    #Why is it already of the form Transition ? 
...
#??????????????????????????????????????????????????????????????????????????????
    ...
    non_final_mask = torch.ByteTensor(tuple(map(lambda s: s is not None,
                                          batch.next_state)))
    global non_final_mask
    #Put 1 when it is final, 0 when it is not
    ...
    non_final_next_states = Variable(torch.cat([s for s in batch.next_state
                                                if s is not None]),
                                     volatile=True)
    global non_final_next_states
#??????????????????????????????????????????????????????????????????????????????

    #Concatenation of the batch:
    state_batch = Variable(torch.cat(batch.state))
    action_batch = Variable(torch.cat(batch.action))
    reward_batch = Variable(torch.cat(batch.reward))

    state_action_values = model(state_batch).gather(1, action_batch)
    #Compute Q(s_t) --> model(state_batch)
    #gather permet d'uniquement sélectionner Q(s_t,a action choisie)
    
    
    next_state_values = Variable(torch.zeros(BATCH_SIZE).type(torch.Tensor))
    next_state_values[non_final_mask] = model(non_final_next_states).max(1)[0]
    #Right : compute max action value for each state
    #Left : with ByteTensor select only position with a 1
